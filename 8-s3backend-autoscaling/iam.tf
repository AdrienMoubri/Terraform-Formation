...
resource "aws_iam_role" "iam_role_s3_access" {
  name = "iam-role-${terraform.workspace}-${var.appname}-s3-access"
  assume_role_policy = data.aws_iam_policy_document.ec2-assume-role-policy.json
}

resource "aws_iam_role" "iam_role_ecs_task_def" {
  name = "iam-role-${terraform.workspace}-${var.appname}-ecs-task-def"
  assume_role_policy = data.aws_iam_policy_document.ecs-assume-role-policy.json
}

resource "aws_iam_user" "iam_user_s3_access" {
  name = "iam-user-${terraform.workspace}-${var.appname}-s3-access"
}
...
resource "aws_iam_policy" "iam_policy_s3_access" {
  name = "iam_policy_${terraform.workspace}_${var.appname}_s3_access"
...
resource "aws_iam_policy" "iam_policy_ecs_access_secret" {
  policy = jsonencode(
    {
    "Version": "2012-10-17",
    "Statement": [
...
resource "aws_iam_role_policy_attachment" "role_attachement_ecs_task_def" {
  role       = aws_iam_role.iam_role_ecs_task_def.name
  policy_arn = aws_iam_policy.iam_policy_ecs_access_secret.arn
}

resource "aws_iam_role_policy_attachment" "role_attachement_ecs_access_secret" {
  role       = aws_iam_role.iam_role_ecs_task_def.name
